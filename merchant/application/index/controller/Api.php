...
class Api extends Indexcommon
{
    ...
    {
        $api=Db::name("user_api")->where("user_id",$this->id)->find();
        if($this->request->isPost()){
            $data=$this->request->param();
            $status=Db::name("user_api")
                ->where("user_id",$this->id)
                ->field("usdtt,usdtc,etht,ethc,btct,btcc")
                ->update($data);
            if($status!==false){
...

    {
        ...
        $data=$list->all();
        $count=$list->total();
        $variate=["name"=>$name,"ip"=>$ip,"type"=>$type,"beigin"=>$beigin,"end"=>$end];
